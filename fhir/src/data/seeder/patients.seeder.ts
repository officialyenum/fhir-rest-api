...
const generatePatientSeedData = async () => {
    // Implement your seed data generation logic here
    for (let i = 0; i < PatientSeeder.length; i++) {
        try {
            const patientCreated = await Patient.build(PatientSeeder[i]);
            await patientCreated.save();
            console.log('Patient created successfully: ', patientCreated.name[0].text);
            console.log(JSON.stringify(patientCreated.identifier));
        } catch (error) {
...
export { PatientSeeder, generatePatientSeedData}